...
> **Three further corrections (2026-08-13), from wiring it up.**
>
> **4. Correction 1 above is stated in terms that do not hold in this codebase, and the gap is the same data-loss bug it was written to close.** It says "skip anything not `ready`" — but `meetings.state = 'ready'` **does not mean a transcript exists**. `fotwd` marks a session `ready` when it finishes whether or not a provider was configured, because recording with transcription switched off is a supported mode (`persist_session` sets it unconditionally). Nor is "a `transcripts` row exists" sufficient: the row is created *before* its segments are appended, so between those two statements — and permanently, if the append failed — the row is there and the text is not. **The only safe predicate is "at least one segment exists"**, and every protection has to key off that rather than off `state`. Keying off `state` left exactly the users who record without a provider exposed: every meeting looked like a transcribed meeting whose audio was a redundant backup, and the first budget sweep would have deleted all of it. `is_evictable` and the eviction-eligibility test in `fotw-pipeline/src/retention.rs` now key off `MeetingAudio::is_transcribed`, and `Db::audio_inventory` is the query that answers it honestly.
>
> **5. §9.2 defines the two locations but nothing ever moved a session between them, and the section reads as if something did.** `sessions/<ulid>/` and `media/<yyyy>/<mm>/<meeting_id>/` both existed on paper; in the shipped code, audio was written to the first and never left it, `media/` was never created, and the retention engine — correct, tested, and never invoked — had nothing to look at. The promotion step is now `fotw-pipeline/src/promote.rs`: claim → encode → publish (`.part` + `rename`) → stamp → verify → retire, resumable after a crash at any point, with the session directory kept as the only copy until every published track has been re-stat'd at the byte length that was recorded.
>
> **6. §9.2's `media/…/{mic.opus.age, system.opus.age}` is aspirational, and what ships is `mic.opus` / `system.opus` in the clear.** §10 specifies media encrypted with `age` (STREAM/ChaCha20-Poly1305) under an HKDF subkey; no `age` dependency exists in the workspace and nothing encrypts a media file today. That is recorded honestly rather than papered over — `recordings.encrypted` is written as `0`, not as the column's default of `1` — but it is a real data-at-rest gap: the library database is SQLCipher-encrypted and the audio beside it is not, so the threat model §10 describes (unencrypted backups, other user-space apps reading the folder) is only half covered. Either build the `age` layer or amend §9.2 and §10 to say audio is plaintext; leaving the `.age` suffix in the layout while writing plaintext is the version that misleads.
...
>
> **1. The Recovery Key must not *be* the master key.** §10 and issue #38 both describe showing "the master key as a Bech32 recovery string". Implemented literally that has two consequences neither text mentions. Rotation becomes impossible: changing the string means changing the key, which means `PRAGMA rekey` over the entire library, so in practice a user whose card was photographed can never revoke it. And the string on the card *is* the live database key, so anyone who glimpses it holds the library forever. The shipped design **wraps** instead: the Recovery Key derives a KEK (Argon2id) that unseals the same 32-byte master key. The library opens identically either way, there is one ciphertext, and rotating rewrites a 200-byte file. §9.7 invariant 7 (the master key must be exportable) is still satisfied — Recovery Key + sealed file *is* the export.
>
> **2. Bech32m, not Bech32.** BIP-350 exists because BIP-173's checksum constant leaves an error in the final data character undetectable in combination with a length change. Fixed-length strings probably dodge it; "probably" is not a word for a data-recovery path.
>
> **3. §10 never says where the recovery material lives, and the obvious answer is wrong.** It cannot be only in the keychain, because the keychain is precisely what is being recovered from — a recovery path stored there is lost with the thing it recovers. It lives in `<root>/db.sqlite3.recovery`, which travels with the database and therefore into every backup the user takes. That has a cost the section should state: **an attacker with disk access holds the wrapped key.** That is exactly why the KDF parameters are not decoration, and it is consistent with the threat model already written down (unencrypted backups; §10.1 puts same-user local malware out of scope).
>
> **4. "Unskippable" is under-specified in the one case that matters.** The section does not say what a run with no human does — CI, launchd, `ssh host fotwd serve`. Silence there resolves to "mint a library whose only key nobody has ever seen", which is the outcome this whole section exists to prevent. A headless first run therefore **refuses**, creating no database, no sealed file and no keychain entry; `FOTW_RECOVERY_UNATTENDED=print-the-key-to-stdout` is the explicit opt-in. And "unskippable" is enforced as *evidence*, not acknowledgement: two randomly chosen groups of the key have to be typed back, because a dialog with one button produces the same outcome as never having shown the key at all.
>
> One thing the paragraph gets exactly right and is worth restating: because the recovery path wraps the **master** key rather than the `db` subkey, adding the HKDF-SHA256 subkey split for `db` and `media` later needs no change here and cannot orphan an existing Recovery Key.
...
| ING-08 | **No cookies, ever.** `localStorage` (keyed by full origin *including port*, paired with the stable default port so tabs share one login) + `Authorization: Bearer` | cookie port-blindness — RFC 6265 scopes by host, so a cookie from `127.0.0.1:51234` is sent to every other localhost service. Zero ambient credentials makes CSRF structurally impossible. |
