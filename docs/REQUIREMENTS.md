...
| STT-11 | **Ordered failover chain** always terminating in the local engine. Demote on 3 failed reconnect cycles, 45 s of no finals while local VAD reports speech, or a non-retryable error. Insert a `ProviderSwitch` marker into the transcript. | P0 | L |
